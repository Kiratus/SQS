...
{
	AutomaticRace = true;
	SetTimerEx("RaceDelay",RESTART*1000,0,"i");
...
{
	//SendClientMessage(playerid, 0xFFFFFFFF, "Hoi!, Este comando ainda está em construção");
...
CMD:icorrida(playerid, params[])
{
    if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, RED, "<!> Você não é admin!");
    if(AutomaticRace == true) return SendClientMessage(playerid, RED, "<!> A corrida automática está ativa!");
    if(BuildRace != 0) return SendClientMessage(playerid, RED, "<!> Tem uma corrida em construção!");
    if(RaceBusy == 0x01 || RaceStarted == 1) return SendClientMessage(playerid, RED, "<!> Há uma corrida em andamento!");
    if(isnull(params)) return SendClientMessage(playerid, RED, "<!> /corrida [nome]");
    LoadRace(playerid, params);
    return 1;
}
CMD:pcorrida(playerid, params[])
{
   	if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, RED, "<!> Você não é admin!");
    if(RaceBusy == 0x00 || RaceStarted == 0) return SendClientMessage(playerid, RED, "<!> Não há corrida em andamento!");
	SendClientMessageToAll(RED, ">> Um admin parou a corrida!");
	return StopRace();
}
CMD:corrida(playerid, params[])
{
	if(RaceStarted == 1) return SendClientMessage(playerid, RED, "<!> A corrida já começou!");
	if(RaceBusy == 0x00) return SendClientMessage(playerid, RED, "<!> Não há corridas em andamento!");
	if(CallRemoteFunction("getDM", "i", playerid) == 1) return SendClientMessage(playerid, RED, "<!> Saia da arena primeiro.");
	if(Joined[playerid] == true) return SendClientMessage(playerid, RED, "<!> Você já está na corrida!");
...
{
	if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, RED, "<!> Você não é admin!");
	if(RaceBusy == 0x01 || RaceStarted == 1) return SendClientMessage(playerid, RED, "<!> Há uma corrida em andamento. Espere terminar!");
	if(AutomaticRace == true) return SendClientMessage(playerid, RED, "<!> Já está habilitada!");
    LoadRaceNames();
	LoadAutoRace(RaceNames[random(TotalRaces)]);
	AutomaticRace = true;
	SendClientMessage(playerid, GREEN, ">> Autorace habilitada!");
	return 1;
}
CMD:stopautorace(playerid, params[])
{
    if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, RED, "<!> Você não é admin!");
    if(AutomaticRace == false) return SendClientMessage(playerid, RED, "<!> Já está desabilitada!");
    AutomaticRace = false;
    SendClientMessage(playerid, GREEN, ">> Autorace desabilitada");
    return 1;
}
CMD:saircorrida(playerid, params[])
{
    if(Joined[playerid] == true)
    {
		...
		#endif
	} else return SendClientMessage(playerid, RED, "<!> Você não está na corrida!");
	return 1;
}
...
public OnPlayerEnterRaceCheckpoint(playerid)
{
	if(CPProgess[playerid] == TotalCP -1)
	{
		...
		format(string, sizeof(string), ">> \"%s\" terminou a corrida na posição \"%d\".", pName, Position);
		SendClientMessageToAll(WHITE, string);
		format(string, sizeof(string), "    - Tempo: \"%d:%d.%d\".", rTime[0], rTime[1], rTime[2]);
		SendClientMessageToAll(WHITE, string);
		format(string, sizeof(string), "    - Prêmio: \"$%d e +%d Score\".", Prize[0], Prize[1]);
		SendClientMessageToAll(WHITE, string);
		...
		if(FinishCount <= 5)
		{
		    ...
		    if(TotalRaceTime <= dini_Int(rFile, string) || TempTotalTime == 0)
		    {
		        ...
		        if(TempTotalTime != 0) format(string, sizeof(string), ">> \"%s\" quebrou um novo recorde de \"%s\" com \"%d\" segundos mais rápido na \"%d\" posição!", pName, dini_Get(rFile, string), -(rTime[1] - TempTime[1]), TimeProgress+1);
					else format(string, sizeof(string), ">> \"%s\" quebrou um novo recorde na \"%d\" posição!", pName, TimeProgress+1);
	...
	switch(CountAmount)
	{
			...
			ForEach(i, MAX_PLAYERS)
			{
			    if(Joined[i] == false)
			    {
			        new
			            string[128]
					;
					format(string, sizeof(string), ">> Você não pode mais entrar na corrida \"%s\". Ela já começou!", RaceName);
	    ...
	    case 60, 50, 45, 20, 10:
	    {
	        new
	            string[128]
			;
			format(string, sizeof(string), ">> \"%d\" segundos até a corrida \"%s\" começar! Digite \"/corrida\" para entrar.", CountAmount, RaceName);
			SendClientMessageToAll(GREEN, string);
	    }
	}
	return CountAmount--;
...
function StopRace()
{
	...
	SendClientMessageToAll(YELLOW, ">> Fim da corrida!");
	if(AutomaticRace == true) SetTimerEx("RaceDelay",RESTART*1000,0,"i");
	return 1;
}
...
function RaceCounter()
{
	if(RaceStarted == 1)
	{
		...
		{
			StopRace();
			SendClientMessageToAll(RED, ">> Corrida acabou. Todos saíram!");
		}
	}
	if(RaceTime <= 0)
	{
...
function TextInfo(playerid)
{
	new
	    string[128]
	;
	if(JoinCount == 1) format(string, sizeof(string), "corrida: ~w~%s~n~~p~~h~Checkpoint: ~w~%d/%d~n~~b~~h~tempo: ~w~%s~n~~y~posicao: ~w~1/1~n~", RaceName, CPProgess[playerid], TotalCP, TimeConvert(RaceTime));
		else format(string, sizeof(string), "corrida: ~w~%s~n~~p~~h~Checkpoint: ~w~%d/%d~n~~b~~h~tempo: ~w~%s~n~~y~posicao: ~w~%d/%d~n~", RaceName, CPProgess[playerid], TotalCP, TimeConvert(RaceTime), RacePosition[playerid], JoinCount);
	TextDrawSetString(RaceInfo[playerid], string);
	TextDrawShowForPlayer(playerid, RaceInfo[playerid]);
}
...
forward IsPlayerOnRace(playerid);
public IsPlayerOnRace(playerid){
